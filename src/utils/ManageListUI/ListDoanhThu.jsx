...
const ListDoanhThu = ({ listBookDoanhThu = [], keySearch, status, statusHeader }) => {
    ...
    const tableRows = currentProducts.map((book, index) => (
        <tr key={index} className='box-shadow_row'
            ...
            <td style={{ width: '120px', textAlign: 'center' }}>{book.da_ban?.toLocaleString('vi-VN')}</td>
    ...
    useEffect(() => {
        ...
        setBookList(listBookDoanhThu);
        const lengthBook = listBookDoanhThu.length;
    ...
    }, [selectedValue, listBookDoanhThu]);
