...
const ListCommentDetails = ({
  ...
  //   Báo cáo button
  const [baoCaoBtn, setBaoCaoBtn] = useState(false);
  ...
  const [cuaHangBaoCao, setCuaHangBaoCao] = useState({});
  const [taiKhoanBiBaoCao, setTaiKhoanBiBaoCao] = useState({});

  // * Ẩn hiện form báo cáo
  const handleShowReportForm = (commentId, taiKhoanBiBaoCao, cuaHangBaoCao) => {
    setShowReportForm(true);
    setReportedComments(commentId);
    setCuaHangBaoCao(cuaHangBaoCao);
    setTaiKhoanBiBaoCao(taiKhoanBiBaoCao);
  };
  const handleCloseReportForm = () => {
    setShowReportForm(false);
    setReportedComments(null);
    setCuaHangBaoCao(null);
    setTaiKhoanBiBaoCao(null);
  };
  ...
  const handleInsertPhanHoiDanhGia = async (idcm, date, content) => {
    const idch = await StoreApi();
    setNotificationStatus("");
    ...
    if (content.length <= 0 || content === undefined || content === null) {
      ...
      try {
        ...
        if (phanHoiDanhGiaNew) {
          NotificationManager.success("Thành công", "Phản hồi đánh giá");
          onReload();
          //   setBaoCaoBtn(false);
        } else {
          NotificationManager.error("Thất bại", "Phản hồi đánh giá");
        }
      } catch (e) {
        NotificationManager.error("Thất bại", "Phản hồi đánh giá");
        console.error(e);
  ...
  const handleDeletePhanHoiDanhGia = async (id) => {
    ...
    try {
      ...
      if (result) {
        NotificationManager.success("Thành công", "Xóa phản hồi đánh giá");
        onReload();
      } else {
        NotificationManager.error("Thất bại", "Xóa phản hồi đánh giá");
      }
    } catch (e) {
      NotificationManager.error("Thất bại", "Xóa phản hồi đánh giá");
      console.error(e);
      ...
      <>
          ...
          <div className="comment" key={index}>
            <div className="comment-flex">
              <div>
                <div className="comment-account">
                  <div className="comment-account_img">
                    <img
                      src={
                        comment.tai_khoan_danh_gia?.anh_dai_dien ||
                  ...
                  <div style={{ marginLeft: "15px" }}>
                    ...
                    <StarRating rating={comment.diem_danh_gia} className="mt-5"/>
              ...
              <div className="comment-report">
                <FontAwesomeIcon
                  onClick={() => setBaoCaoBtn(!baoCaoBtn)}
                  ...
                  <button
                    onClick={() =>
                      handleShowReportForm(
                        comment.ma_danh_gia,
                        comment.tai_khoan_danh_gia,
                        comment.don_hang_chi_tiet?.san_pham?.cua_hang
                      )
                    }
            ...
            <div className="seller-feedback">
              ...
              <OverlayTrigger
                ...
                placement="right"
                overlay={(props) =>
                  renderTooltip(props, "Xóa phản hồi", "tooltip-del-feedback")
                }
              >
                <FontAwesomeIcon
                  onClick={() =>
                    handleDeletePhanHoiDanhGia(feedback.ma_phan_hoi)
                  }
        ...
        {!feedback && (
          <OverlayTrigger
            ...
            placement="right"
            overlay={(props) =>
              renderTooltip(props, "Bấm vào đánh giá để phản hồi", "tooltip2")
            }
          >
            <div
              ...
              {!feedback && showFeedbackInput === comment.ma_danh_gia && (
                <div className="reply">
                  <input
                    ...
                    placeholder="Phản hồi khách hàng của bạn"
                    onChange={handleChangeContentPhanHoi}
                  />
                  <button
                    onClick={() =>
                      handleInsertPhanHoiDanhGia(
                        commentID,
    ...
    <div className="update-width-list">
      ...
      {showReportForm && (
        <ReportForm
          ...
          IDComment={reportedComments}
          taiKhoanBiBaoCao={taiKhoanBiBaoCao}
          cuaHangBaoCao={cuaHangBaoCao}
        />
      )}
...
export default ListCommentDetails;
